...
#Setting up Hostname
MyHostName=$(hostname)
#Setting up LAN information
MyInterface=$(ip a |awk '/: e/{gsub(/:/,"");print $2}')
MyLanAddress=$(ip a s $MyInterface|awk '/inet /{gsub(/\/.*/,"");print $2}')
MyLanHost=$(getent hosts $MyLanAddress| awk '{print $2}')
#External IP address information
externalIpAddress=$(curl -s icanhazip.com)
MyExternalName=$(getent hosts $externalIpAddress| awk '{print $2}')
...
#Task2
#Router information
MyRouter=$(ip r s default|  awk '{print $3}')
MyRouterName=$(getent hosts $MyRouter| awk '{print $2}')

cat <<EOF
Hostname        : $MyHostName
LAN Address     : $MyLanAddress
LAN Hostname    : $MyLanHost
External IP     : $externalIpAddress
External Name   : $MyExternalName
Router Address  : $MyRouter
